...
params <- c("mu","alpha","beta1","delta", "gamma0", "gamma1", "gamma2","sigma1", "sigma2", "z")
...
MCMCsummary(jagsfit, params = c("alpha", "beta1", "delta", "gamma0", "gamma1", "gamma2", "sigma1", "sigma2"))
...
betas<-MCMCpstr(jagsfit, params=c("alpha", "beta1", "delta", "gamma0",  "gamma1", "gamma2"), type="chains")
...
for(i in 1:nlats){
  ...
  mig2_hats <- betas$gamma0 + betas$gamma1*rep(lats_pred[i],nmcmc)+
    betas$gamma2*(rep(lats_pred[i],nmcmc)-betas$delta)*I(rep(lats_pred[i],nmcmc)-betas$delta > 0)
...
betas_hat<-MCMCpstr(jagsfit, params = c("alpha", "beta1", "delta","gamma0",  "gamma1","gamma2", "z"), func=mean)
...
mu_hats2<-data.frame(est=betas_hat$gamma0+betas_hat$gamma1*lats_pred+
