...
write_csv(res, here("rerun5_var_output/best_mod_params.csv"))
...
model_comparison<-data.frame(id=NA, year=NA, 
                             ...
                             six_int_loo=NA,
                             seven_int_loo=NA)
write_csv(model_comparison, here("rerun5_var_output/model_comparisons.csv"))
    ...
    if(nrow(tmp_yr)>30){
      ...
      out_mods<-foreach(mm=1:length(int_mods), .errorhandling = 'pass')%dopar%{
        ...
        tryCatch(out_mods[[mm]]<-mcp(model = int_mods[[mm]], 
                                     ...
                                     iter=25000), # first run was default (3000)
        ...
        if(length(out_mods[[nn]])!=0){
          if(out_mods[[nn]]$rhat_fail==F){
            ...
            cat("Skipped model ", years[[j]], int_mod_vec[[k]], "\n",
                file=here("rerun5_var_output/skipped_mods.txt"), append=T)
      ...
      # write out to file
      write_csv(as.data.frame(t(mods)), here("rerun5_var_output/model_comparisons.csv"), append = T)
      ...
      # write out to file
      write_csv(params, here("rerun5_var_output/best_mod_params.csv"), append = T)
      ...
      ggsave(plot = p, filename = here(glue::glue("rerun5_var_output/best_mod_plots/{years[[j]]}.png")))
      ...
      cat("Dataset for ", years[[j]], "skipped because of insufficient sample size.\n",
          file=here("rerun5_var_output/skipped_swan_years.txt"), append=T)
    }}
  # keep track of progress
