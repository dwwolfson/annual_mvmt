...
params <- c("mu","alpha","beta1","gamma0", "gamma1", "gamma2","sigma", "z")
...
betas<-MCMCpstr(jagsfit, params=c("alpha", "beta1", "gamma0", "gamma1", "gamma2"), type="chains")

lats_pred<-seq(from=min(df$breeding_lat),
               to=max(df$breeding_lat),
               length=100)
...
for(i in 1:nlats){
  ...
  mig2_hats <- betas$gamma0+rep(lats_pred[i], nmcmc)*betas$gamma1+(rep(lats_pred[i], nmcmc)^2)*betas$gamma2
  
  conf.int1[i,] <- quantile(mig1_hats, prob = c(0.025, 0.975))
  conf.int2[i,] <- quantile(mig2_hats, prob = c(0.025, 0.975))
}

betas_hat<-MCMCpstr(jagsfit, params = c("alpha", "beta1", "gamma0", "gamma1", "gamma2"), func=mean)
...
mu_hats2<-data.frame(est=rep(betas_hat$gamma0, nlats)+
                       rep(betas_hat$gamma1, nlats)*lats_pred+
                       rep(betas_hat$gamma2,nlats)*lats_pred^2,
                     LCL=conf.int2[,1],
                     UCL=conf.int2[,2],
                     latitudes=lats_pred)
