...
Spring onset (departure from furthest extent segment)
```{r}
p_dates %>% 
  filter(breeding_status%in%c("breeder", "non_breeder", "paired")&
           furthest_seg_departure < '2020-05-01') %>% 
  ggplot(., aes(breeding_status, furthest_seg_departure, fill=breeding_status))+
  geom_boxplot(width=0.2, outlier.shape = NA)+
  geom_jitter(width=0.2,alpha=0.9)+
  scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07"))+
  ggtitle(label="Spring Migration Onset by Breeding Status",
          subtitle="Lots of overlap, but non-breeders seem to leave earlier on average")+
  theme(plot.title = element_text(hjust=0.5, size=14),
        plot.subtitle = element_text(hjust=0.5, size=12))+
  labs(x="\nBreeding Status", y="\nDate of Spring Migration Onset")
...
Could look at legnth of fall migration (arrival-onset) and length of spring migration (arrival-onset) for lat and breeding class.

Also, consider adding in plots that account for both timing and extent with either lat or breeding class (or both)

Plot showing the timing of migration with beginning lat and ending lat on the y axis and time on the x (or flipped). would look like spaghetti plots

Assuming there are 2 different movement strategies going on, are they happening in parallel (i.e. same time) or at different times?
```{r}
p_dates %>% 
  filter(id_year!="9N-2021-2022" & id_year!="6M-2021-2022") %>% 
  ggplot(., aes(fall_mig_onset,mig_extent))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~entire_yr)+
  ggtitle(label="Fall onset vs extent",
          subtitle="Swans that left later traveled further")+
  theme(plot.title = element_text(hjust=0.5, size=14),
        plot.subtitle = element_text(hjust=0.5, size=12))+
  labs(x="Fall Onset of migration", y="\nMigration extent (in km)")
